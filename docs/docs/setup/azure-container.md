...
3. In the container tab, uncheck the "Use quickstart image" option to select the image source manually.
  ...
  **Note: For the minimal setup, ToolJet requires: `TOOLJET_HOST`, `POSTGRES_HOST`, `POSTGRES_DB`, `POSTGRES_USER`, `PG_PASSWORD`, `SECRET_KEY_BASE` & `LOCKBOX_MASTER_KEY` keys in the secret.**
