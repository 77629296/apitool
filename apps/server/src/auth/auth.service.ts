...
import { ErrorMessage } from '@api-code/utils';
...
export class AuthService {
  ...
  async forgotPassword(email: string) {
    ...
    const subject = 'Reset your APICode password';
  ...
  async setup2FASecret(email: string) {
    ...
    const uri = authenticator.keyuri(email, 'APICode', secret);
  ...
  async useBackup2FACode(email: string, code: string) {
    ...
    const backupCodes = user.secrets?.twoFactorBackupCodes.filter(
      (c) => c !== code,
    );
    await this.userService.updateByEmail(email, {
      secrets: { update: { twoFactorBackupCodes: backupCodes } },
