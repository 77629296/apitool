...
export const translateError = (error: ErrorMessage) => {
  switch (error) {
    ...
    case ErrorMessage.SlugAlreadyExists:
      return t`A apitool with this slug already exists, please pick a different unique identifier.`;
    case ErrorMessage.NotFound:
      return t`It looks like the apitool you're looking for doesn't exist.`;
